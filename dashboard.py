    ...
    def _view_data_page(self):
        ...
        with col3:
            department = st.selectbox(
                 "Department",
                ["All"] + sorted(data["department"].unique().tolist()),
                index=st.session_state.get("department_index", 0)
            )
        
        # Apply filters
        filtered_data = data
        filtered_data = filtered_data[(filtered_data["risk_score"] >= min_risk) & 
                                (filtered_data["risk_score"] <= max_risk)]
        ...
        with viz_tab3:
            ...
            time_agg = time_data.resample('D').agg({
                'amount': 'sum',
                'risk_score': 'mean',
                'transaction_id': 'count'
            }).rename(columns={'transaction_id': 'count'}).reset_index()
    ...
    def _run_audit_page(self):
        ...
        with col1:
            audit_type = st.selectbox(
                "Audit Type",
                ["Comprehensive", "Risk-Based", "Department-Specific", "Custom"]
            )
        ...
        with st.expander("Configure Audit Rules"):
            ...
            custom_rule = st.text_area("Add custom rule (Python expression)", 
                                      "transaction['amount'] > 10000 and transaction['approvals'] < 2")
        ...
        if st.button("Run Audit", type="primary"):
            ...
            audit_params = {
                ...
                "rules": {
                    "round_numbers": rule1,
                    "below_threshold": rule2,
                    "weekend_holiday": rule3,
                    "documentation": rule4,
                    "custom": custom_rule if custom_rule else None
                }
            }
            if "processed_data" not in st.session_state:
            ...
            try:
                ...
                with st.spinner("Running audit analysis..."):
                    try:                        
                        ...
                        audit_results = self.audit_agent.run_manual_audit(
                            start_date=audit_params["start_date"],
                            end_date=audit_params["end_date"],
                            audit_type=audit_params["audit_type"],
                            department=department_value,
                            risk_threshold=audit_params["risk_threshold"],
                            sample_size=audit_params["sample_size"],
                            rules=audit_params["rules"]
                        )
                    ...
                    # if audit_params["audit_type"] == "Department-Specific" and "department" in audit_results:
                    #     filtered_results = audit_results[audit_results["department"] == audit_params["department"]]
                    #     audit_results = filtered_results
                    # st.session_state["audit_results"] = audit_results
                    # st.success(f"Audit completed successfully. Found {len(audit_results.get('flagged_transactions', []))} issues.")
    ...
    def _configure_monitoring_page(self):
            ...
            with col2:
                ...
                if trigger_pattern:
                    pattern_options = st.multiselect(
                        "Select Patterns to Monitor",
                        ["Round Numbers", "Split Transactions", "After Hours", "Duplicate Transactions"],
                        default=["Round Numbers", "Split Transactions"]
                    )
        ...
        if st.button("Save Configuration", type="primary"):
            ...
            config = {
                "schedule": {
                    "type": schedule_type,
                    "interval_minutes": interval_minutes if schedule_type == "Continuous" else None,
                    "time_of_day": time_of_day.strftime("%H:%M") if schedule_type in ["Daily", "Weekly", "Monthly"] else None,
                    "day_of_week": day_of_week if schedule_type == "Weekly" else None,
                    "day_of_month": day_of_month if schedule_type == "Monthly" else None
                },
                "triggers": {
                    "risk_score": {
                        "enabled": trigger_risk_score,
                        "threshold": risk_threshold if trigger_risk_score else None
                    },
                    "amount": {
                        "enabled": trigger_amount,
                        "threshold": amount_threshold if trigger_amount else None
                    },
                    "velocity": {
                        "enabled": trigger_velocity,
                        "threshold": velocity_threshold if trigger_velocity else None
                    },
                    "patterns": {
                        "enabled": trigger_pattern,
                        "selected_patterns": pattern_options if trigger_pattern else []
                    }
                },
                "notifications": {
                    "channels": notification_type,
                    "email": {
                        "recipients": recipients.split("\n") if "Email" in notification_type else [],
                        "subject": email_subject if "Email" in notification_type else ""
                    },
                    "sms": {
                        "phone_numbers": phone_numbers.split("\n") if "SMS" in notification_type else []
                    },
                    "webhook": {
                        "url": webhook_url if "Webhook" in notification_type else "",
                        "auth": webhook_auth if "Webhook" in notification_type else ""
                    }
                },
                "advanced": {
                    "log_level": log_level,
